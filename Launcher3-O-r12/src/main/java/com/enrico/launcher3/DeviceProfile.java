...
import com.enrico.launcher3.CellLayout.ContainerType;
...
public class DeviceProfile {
    ...
    float workspaceSpringLoadShrinkFactor;
    final int workspaceSpringLoadedBottomSpace;
    ...
    // Folder child
    int folderChildIconSizePx;
    int folderChildTextSizePx;
    int folderChildDrawablePaddingPx;

    // Hotseat
    int hotseatCellWidthPx;
    private int hotseatCellHeightPx;
    private int hotseatIconSizePx;
    ...
    int allAppsIconDrawablePaddingPx;
    float allAppsIconTextSizePx;

    // Widgets
    final PointF appWidgetScale = new PointF(1.0f, 1.0f);

    // Drop Target
    int dropTargetBarSizePx;
    ...
    void addLauncherLayoutChangedListener(LauncherLayoutChangeListener listener) {
        if (!mListeners.contains(listener)) {
            mListeners.add(listener);
        }
    }

    void removeLauncherLayoutChangedListener(LauncherLayoutChangeListener listener) {
        if (mListeners.contains(listener)) {
     ...
     */
    Rect getWorkspacePadding(Rect recycle) {
    ...
    int getCellHeight(@ContainerType int containerType) {
    ...
    boolean shouldIgnoreLongPressToOverview(float touchX) {
