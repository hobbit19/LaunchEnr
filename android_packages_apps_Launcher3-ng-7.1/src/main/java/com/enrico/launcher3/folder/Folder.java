...
import com.enrico.launcher3.pageindicators.PageIndicatorDots;
...
public class Folder extends LinearLayout implements DragSource, View.OnClickListener,
    ...
    //Notifications shit
    static Map<FolderInfo, ArrayList<ShortcutInfo>> HAS_CONTENT_NOTIFICATION = new HashMap<>();
    HashSet<ShortcutInfo> packages;

    //determine if the folder has packages with notifications
    static boolean hasShortcutsWithNotifications(FolderInfo folderInfo) {

        boolean tmp = false;
        if (HAS_CONTENT_NOTIFICATION.get(folderInfo) != null) {
            tmp = HAS_CONTENT_NOTIFICATION.get(folderInfo).size() != 0;
        }
    ...
    protected void onFinishInflate() {
        ...
        //set theme attributes for footer and text color
        int defaultColor = Utilities.isDarkThemeEnabled(mLauncher)? ContextCompat.getColor(mLauncher, R.color.material_dark) : ContextCompat.getColor(mLauncher, R.color.material_light_o);
        int color = Utilities.areColoredFoldersEnabled(mLauncher)? Utilities.getColorAccent(mLauncher): defaultColor;
        mFooter.setBackgroundColor(color);
        int textDefaultColor = Utilities.isDarkThemeEnabled(mLauncher)? ContextCompat.getColor(mLauncher, R.color.material_light) : ContextCompat.getColor(mLauncher, R.color.material_dark);
        int textColor = Utilities.areColoredFoldersEnabled(mLauncher)? defaultColor : textDefaultColor;
    ...
    void bind(FolderInfo info) {
        ...
        for (ShortcutInfo shortcutInfo: children) {

            //if any package inside the folder has notifications add to the packages hashset
            if (NotificationsDotListener.hasNotifications(shortcutInfo.getTargetComponent().getPackageName())) {

                packages.add(shortcutInfo);
            }
        }

        //for every folder create an array list with the list of packages showing notifications
        HAS_CONTENT_NOTIFICATION.put(info, new ArrayList<>(packages));
