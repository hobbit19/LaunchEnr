package com.enrico.launcher3.folder;
...
import com.enrico.launcher3.CheckLongPressHelper;
import com.enrico.launcher3.DeviceProfile;
import com.enrico.launcher3.DropTarget.DragObject;
import com.enrico.launcher3.FastBitmapDrawable;
import com.enrico.launcher3.FolderInfo;
import com.enrico.launcher3.FolderInfo.FolderListener;
...
public class FolderIcon extends FrameLayout implements FolderListener {
    ...
    public static class PreviewBackground {
        ...
        private static final int BG_INTENSITY_DARK = 48;
        ...
        public void drawBackground(Context context, Canvas canvas, Paint paint) {
            ...
            int accent = Utilities.getColorAccent(context);
            int defaultColor = Utilities.isDarkThemeEnabled(context)? Color.argb(alpha, BG_INTENSITY_DARK, BG_INTENSITY_DARK, BG_INTENSITY_DARK) : Color.argb(alpha, BG_INTENSITY, BG_INTENSITY, BG_INTENSITY);
            int color = Utilities.areColoredFoldersEnabled(context)? ColorUtils.setAlphaComponent(accent, alpha) : defaultColor;
        ...
        public void drawBackgroundStroke(Context context,Canvas canvas, Paint paint) {
            canvas.save();
            canvas.translate(getOffsetX(), getOffsetY());
            ...
            int defaultColor = Utilities.isDarkThemeEnabled(context)? Color.argb(255, BG_INTENSITY_DARK, BG_INTENSITY_DARK, BG_INTENSITY_DARK) : Color.argb(255, BG_INTENSITY, BG_INTENSITY, BG_INTENSITY);

            int color = Utilities.areColoredFoldersEnabled(context)? Utilities.getColorAccent(context) : defaultColor;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (!mBackground.drawingDelegated()) {
            mBackground.drawBackground(context, canvas, mBgPaint);
        }
        ...
        if (mPreviewLayoutRule.clipToBackground() && !mBackground.drawingDelegated()) {
            mBackground.drawBackgroundStroke(context, canvas, mBgPaint);
        }

        //if the folder has content with notifications draw a new layer showing a message alert icon
        if (Folder.HAS_CONTENT_NOTIFICATION.get(getFolderInfo()) != null) {

            if (Folder.hasShortcutsWithNotifications(getFolderInfo())) {

              IconUtils.drawNotificationBackground(context, canvas, mBackground.getOffsetX(), mBackground.getOffsetY(), mBackground.mClipPath, mBackground.getStrokeWidth(), mBackground.getScaledRadius(), PreviewBackground.SHADOW_OPACITY);
            }
        }
    ...
    private class FolderPreviewItemAnim {
         ...
         */
        FolderPreviewItemAnim(final PreviewItemDrawingParams params, int index0, int nItems0,
        ...
        boolean hasEqualFinalState(FolderPreviewItemAnim anim) {
    ...
    public void onAdd(ShortcutInfo item) {
        ...
        //if the added item has notifications add to the array list to notify the folder
        if (NotificationsDotListener.hasNotifications(item.getTargetComponent().getPackageName())) {

            Folder.HAS_CONTENT_NOTIFICATION.get(getFolderInfo()).add(item);
        }
    ...
    //if the item remove has notifications notify the folder by removing it from the array list
    public void onRemove(ShortcutInfo item) {

        if (NotificationsDotListener.hasNotifications(item.getTargetComponent().getPackageName())) {

            Folder.HAS_CONTENT_NOTIFICATION.get(getFolderInfo()).remove(item);
        }
