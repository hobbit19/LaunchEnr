...
class AllAppsGridAdapter extends RecyclerView.Adapter<AllAppsGridAdapter.ViewHolder> {
    ...
    static final int VIEW_TYPE_SECTION_BREAK = 1;
    // A normal icon
    static final int VIEW_TYPE_ICON = 1 << 1;
    // A prediction icon
    static final int VIEW_TYPE_PREDICTION_ICON = 1 << 2;
    // The message shown when there are no filtered results
    static final int VIEW_TYPE_EMPTY_SEARCH = 1 << 3;
    // The message to continue to a market search when there are no filtered results
    static final int VIEW_TYPE_SEARCH_MARKET = 1 << 4;
    // A divider that separates the apps list and the search market button
    static final int VIEW_TYPE_SEARCH_MARKET_DIVIDER = 1 << 5;

    // We use various dividers for various purposes.  They share enough attributes to reuse layouts,
    // but differ in enough attributes to require different view types
    // The divider under the search field
    static final int VIEW_TYPE_SEARCH_DIVIDER = 1 << 6;
    // The divider that separates prediction icons from the app list
    static final int VIEW_TYPE_PREDICTION_DIVIDER = 1 << 7;
    // Common view type masks
    private static final int VIEW_TYPE_MASK_DIVIDER = VIEW_TYPE_SEARCH_DIVIDER
            | VIEW_TYPE_SEARCH_MARKET_DIVIDER
            | VIEW_TYPE_PREDICTION_DIVIDER
            | VIEW_TYPE_SECTION_BREAK;
    private static final int VIEW_TYPE_MASK_ICON = VIEW_TYPE_ICON
    ...
    AllAppsGridAdapter(Launcher launcher, AlphabeticalAppsList apps, View.OnClickListener
    ...
    static boolean isDividerViewType(int viewType) {
        return isViewType(viewType, VIEW_TYPE_MASK_DIVIDER);
    }

    static boolean isIconViewType(int viewType) {
    ...
    void setSearchController(AllAppsSearchBarController searchController) {
     ...
     */
    void setBindViewCallback(BindViewCallback cb) {
     ...
     * item decorator.
     */
    void updateBackgroundPadding(Rect padding) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        switch (viewType) {
            ...
            case VIEW_TYPE_SEARCH_MARKET_DIVIDER:
                ...
                View divider = mLayoutInflater.inflate(
                        R.layout.all_apps_divider, parent, false);

                int accent = Utilities.getColorAccent(mLauncher);
                int opaqueAccent = ColorUtils.setAlphaComponent(accent, 100);

                divider.setBackgroundColor(opaqueAccent);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        switch (holder.getItemViewType()) {
            ...
            case VIEW_TYPE_EMPTY_SEARCH:
                ...
                int emptyViewTextColor = Utilities.isDarkThemeEnabled(mLauncher.getBaseContext()) ? ContextCompat.getColor(mLauncher.getBaseContext(), R.color.material_light) : ContextCompat.getColor(mLauncher.getBaseContext(), R.color.material_dark);
                emptyViewText.setTextColor(emptyViewTextColor);
     ...
     */
    private class AppsGridLayoutManager extends GridLayoutManager {
        ...
        public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
            ...
            // adapter views
            final AccessibilityRecord record = AccessibilityEvent.obtain();
