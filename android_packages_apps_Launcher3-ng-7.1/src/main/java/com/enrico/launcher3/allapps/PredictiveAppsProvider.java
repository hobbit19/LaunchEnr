...
public class PredictiveAppsProvider {
    ...
    public void updateComponentCount(ComponentName component) {
        ...
        sharedPreferences.edit().putLong(key, current + 1).apply();
        ...
        if (!predictiveApps.contains(key)) {
            ...
            sharedPreferences.edit().putStringSet(PREDICTIVE_APPS_KEY, predictiveApps).apply();
        }
    }
            ...
            public void run() {
                ...
                Collections.sort(allPredictions, new Comparator<PredictedApp>() {
                    public int compare(PredictedApp result1, PredictedApp result2) {
                        return Long.valueOf(result2.count).compareTo(result1.count);
                    }
                });
                ...
                sharedPreferences.edit().putString(TOP_PREDICTIVE_APPS_KEY, buildStringFromAppList(allPredictions)).apply();
            }
        }).start();
    }
    ...
    private ComponentKey buildComponentKey(ComponentName component) {
        return new ComponentKey(component, UserHandleUtil.myUserHandle());
    }
    ...
    private class PredictedApp {
        ...
        PredictedApp(ComponentName component, long count) {
